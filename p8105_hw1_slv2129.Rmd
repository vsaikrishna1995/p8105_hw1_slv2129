...
The penguins dataset consists of the following variables: `r names(penguins)`. The penguin species associated with this respective dataset are `r levels(penguins$species)`. Among these species, the gender distribution is as follows: `r summary(penguins$sex)`. This data has been recorded on the penguin species from the `r levels(penguins$island)` islands. This dataset comprises of `r nrow(penguins)` rows and `r ncol(penguins)` columns. The mean flipper length of the penguins is observed as `r mean(penguins$flipper_length_mm). 
